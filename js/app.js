...
var CELULAR_EMPRESA = '86999343379';
...
cardapio.eventos = {
    ...
    init: () => {
        ...
        cardapio.metodos.carregarBtnWpp();
    }
...
cardapio.metodos = {
    ...
    adicionarAoCarrinho: (id) => {
        ...
        if (qntAtual > 0) {
            ...
            if (item.length > 0) {
                ...
                //Validar se ja esxiste esse item no carrinho 
                let existe = $.grep(MEU_CARRINHO, (elem, index) => { return elem.id == id })

                //Caso exista o item no carrinho, so altera a quantidade.
                if (existe.length > 0) {
    ...
    //Abrir e fechar a modal do carrinho.
    abrirCarrinho: (abrir) => {
    ...
    //Vai carregar as etapas para cliente poder finalizar e revisar seu pedido 
    carregarEtapa: (etapa) => {
    ...
    //diminuir quantidade no item no carrinho
    ...
    //aumentar quantidade no item no carrinho
    ...
    atualizarCarrinho: (id, qntd) => {
        ...
        //Atualiza o botao de carrinho com a quantidade atualizada
        cardapio.metodos.atualizarBadgeTotal();
    ...
    buscarCep: () => {
        ...
        if(cep != ""){
            //Expressao para regular validar o CEP
            var valicacep = /^[0-9]{8}$/;

            if(valicacep.test(cep)){
                ...
                $.getJSON("https://viacep.com.br/ws/" + cep + "/json/?callback=?", function(dados){
                    ...
                    if(!("erro" in dados)){
                        ...
                        cardapio.metodos.mensagem('CEP não encontrado preencha manualmente.');
                        $("#txtEndereco").focus();
                    }
            ...
            cardapio.metodos.mensagem('Informe Seu CEP, Porfavor');
            $('#txtCEP').focus();
        }
    ...
    //Atualiza o link botão do whatsApp
    finalizarPedido: () =>{
        ...
        if(MEU_CARRINHO.length > 0 && MEU_ENDERECO != null){
            ...
            $.each(MEU_CARRINHO, (i, e) => {
                ...
                // https://wa.me/5586999343379?text=Ola
            });
    ...
    //carregar link do botao reserva.
    ...
    //Manda uma mensagem quando adiciona item no carrinho.
    mensagem: (texto, cor = 'red', tempo = 3500) => {

        let id = Math.floor(Date.now() * Math.random()).toString();

        let msg = `<div id="msg-${id}" class="animated fadeInDown toasd ${cor}">${texto}<div/>`;
